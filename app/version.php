...
if ( ! function_exists( 'coffee_code_challenge_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function coffee_code_challenge_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'coffee_code_challenge_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function coffee_code_challenge_should_load_wpemerge( $name, $min, $max ) {
		$loaded = coffee_code_challenge_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'coffee_code_challenge' );
				?>
...
if ( ! function_exists( 'coffee_code_challenge_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function coffee_code_challenge_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = coffee_code_challenge_get_loaded_wpemerge();
