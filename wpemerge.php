<?php
/**
 * Plugin Name: Coffee Code Challenge
 ...
 * Text Domain: coffee_code_challenge
...
$load = coffee_code_challenge_should_load_wpemerge( $name, '0.17.0', '2.0.0' );

if ( ! $load ) {
	// An incompatible WP Emerge version is already loaded - stop further execution.
	// coffee_code_challenge_should_load_wpemerge() will automatically add an admin notice.
	return;
}

define( 'COFFEE_CODE_CHALLENGE_PLUGIN_FILE', __FILE__ );
...
coffee_code_challenge_declare_loaded_wpemerge( $name, 'theme', __FILE__ );

// Load helpers.
require_once __DIR__ . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'CoffeeCodeChallenge.php';
...
\CoffeeCodeChallenge::make()->bootstrap( require __DIR__ . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'config.php' );
